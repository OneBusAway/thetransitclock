...
- **transitclockIntegration** — end-to-end / prediction-accuracy tests; only built under the `include-integration-tests` profile. Fixture-refresh workflow (capture, subset, promote, regenerate baselines) is documented in `integration-tests.md` at the repo root; supporting tooling under `tools/wmata_capture/`.
