...
<%-- Cache busters: tied to file mtime so edits propagate without forcing
     users to hard-refresh. getRealPath returns null for packed WARs, where
     the buster degrades to ?v=0 (still cacheable; busts only on redeploys
     that exploded the WAR). --%>
<%
String cssPath = application.getRealPath("/css/general.css");
long cssVer = cssPath != null ? new java.io.File(cssPath).lastModified() : 0L;
String jsPath = application.getRealPath("/javascript/transitime.js");
long jsVer = jsPath != null ? new java.io.File(jsPath).lastModified() : 0L;
%>
<link rel="stylesheet" href="<%= request.getContextPath() %>/css/general.css?v=<%= cssVer %>">

<script src="<%= request.getContextPath() %>/javascript/transitime.js?v=<%= jsVer %>"></script>
